...
export function MainSiteNavigation() {
  ...
  const handleEnterpriseClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Close dropdown on click (state only)
  ...
  return (
    <nav className="fixed bg-[rgba(255,255,255,0.95)] h-[65px] left-0 top-0 w-full z-50 backdrop-blur-sm border-b border-[#e5e7eb]">
      <div className="flex h-full items-center justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-24">
        ...
        {/* Mobile hamburger (static-export friendly) */}
        <button
          type="button"
          aria-label="Open menu"
          aria-controls="mainsite-mobile-menu"
          data-mobile-toggle
          className="md:hidden inline-flex items-center justify-center rounded p-2 hover:bg-[#f0f7ff] transition-colors"
          onClick={() => {
            // Dev-mode convenience only (static export uses DOM scripting)
            setProgramsOpen(false);
            setAboutOpen(false);
          }}
        >
      ...
      {/* Mobile menu panel (static-export friendly: always rendered, toggled via main.js) */}
      <div
      ...
      >
        ...
        {/* Panel */}
        <div className="absolute left-0 right-0 top-[65px] bg-white border-b border-[#e5e7eb] shadow-lg">
